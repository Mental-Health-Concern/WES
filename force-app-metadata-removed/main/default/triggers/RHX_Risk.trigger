...
**/
trigger RHX_Risk on Risk__c(after delete, after insert, after undelete, after update, before delete ){
	...
	if (runningUser.skip_triggers__c != true){
		...
		if (rollClass != null){
			...
			if (trigger.isAfter){
				pu.performTriggerRollups(trigger.oldMap, trigger.newMap, new String[]{'Risk__c'}, null);
			}
		}
	}
}