...
**/
trigger RHX_Appointment on Appointment__c(after delete, after insert, after undelete, after update, before delete ){
	...
	if (runningUser.skip_triggers__c != true){
		...
		if (rollClass != null){
			...
			if (trigger.isAfter){
				pu.performTriggerRollups(trigger.oldMap, trigger.newMap, new String[]{'Appointment__c'}, null);
			}
		}
	}
}