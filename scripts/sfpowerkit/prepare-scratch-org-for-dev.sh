...
call sfdx force:source:push -u $username -r

# Assign all permission sets and permission set groups specified by name in config/wes-permission-sets-for-admin.txt
...
while read permSetName; do
    ...
    call sfdx force:user:permset:assign -u $username --permsetname $permSetName
done <config/wes-permission-sets-for-admin.txt